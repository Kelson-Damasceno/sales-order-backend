...
export class SalesOrderHeaderServiceImpl implements SalesOrderHeaderService {
    ...
    public async beforeCreate(params: SalesOrderHeader): Promise<CreationPayloadValidationResult> {
        ...
        const headerValidationResult = header.validateCreationPayLoad({ 
            customer_id:( customerValidationResult.customer as unknown as CustomerModel).id
        });
    ...
    private async validateProductsOnCreation(
        ...
        const products = await this.getProductsByIds(header);
            if (!products) {
                return {
                    hasError: true,
                    error: new Error('Nenhum produto da lista foi encontrado.')
                };
            }
        return {
            hasError: false,
    ...
    private async getProductsByIds(params: SalesOrderHeader | BulkCreateSalesOrderPayload
